...
const normalizeBlockingObservation = (rawObs) => {
    ...
    return {
        ...
        start: toISODateString(startDate), end: toISODateString(endDate),
        rawStart: startDate, rawEnd: endDate
    };
};
            ...
            for (const day of candidates) {
                ...
                if (extraRemoved > 0 || baseRemoved > 0) {
                    ...
                    diasAjustadosEnSemana.push({
                        fecha: day.fecha, banco_id: entry.id,
            ...
            if (weekAppliedHours > 0) {
                ...
                if (!weekSummariesMap.has(summaryKey)) {
                    weekSummariesMap.set(summaryKey, {
                        semana_inicio: week.fecha_inicio, semana_fin: week.fecha_fin, dias: [],
                    });
                }
    ...
    try {
        ...
        for (const dayDataFromFrontend of parsedDays) {
            ...
            if (regularCap > 0) {
                 ...
                 const prevOverRegular = Math.max(0, toFixedNumber(previousHours - regularCap));
                 const newOverRegular = Math.max(0, toFixedNumber(totalHours - regularCap));
                 manualOvertimeTotal = toFixedNumber(manualOvertimeTotal + newOverRegular);
            ...
            if (totalHours > 0 && wd !== 7) {
                const dayInfo = getDayInfo(wd, false, null, Boolean(day.jornada_reducida), day.tipo_jornada_reducida || 'salir-temprano');
                const { blocks, entryTime, exitTime } = allocateHoursRandomly(day.fecha, dayInfo, totalHours);
                day.bloques = blocks;
                day.jornada_entrada = entryTime;
                day.jornada_salida = exitTime;
            } else {
        ...
        const manualOvertimeToRegister = Math.max(0, toFixedNumber(manualOvertimeDeltaNeto - Math.max(0, weeklyExcesoDelta)));
