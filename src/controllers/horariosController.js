...
export const getHorariosByEmpleadoId = async (req, res) => {
    ...
    try {
        ...
        const { data } = await supabaseAxos.get(url);
        res.json(data);
    } catch (e) {
...
export const createHorario = async (req, res) => {
    try {
        ...
        const { data: [empleado], error: empleadoError } = await supabaseAxios.get(
            `/empleados?select=nombre_completo,email&id=eq.${empleado_id}`
        );
        if (empleadoError || !empleado || !empleado.email) {
            console.error("No se pudo obtener el email del empleado. El correo no se enviará.");
        } else {
    ...
    try {
        ...
        if (legalSum > WEEKLY_LEGAL_LIMIT + 1e-6) {
            return res
                .status(400)
                .json({
                    message: `Excede ${WEEKLY_LEGAL_LIMIT}h legales semanales (${legalSum.toFixed(2)}h).`,
                });
        }
        if (extraSum > WEEKLY_EXTRA_LIMIT + 1e-6) {
            return res
                .status(400)
                .json({
                    message: `Excede ${WEEKLY_EXTRA_LIMIT}h extras semanales (${extraSum.toFixed(2)}h).`,
                });
        }
